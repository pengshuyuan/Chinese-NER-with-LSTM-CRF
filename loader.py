...
def load_sentences(path, lower, zeros):
    ...
    """
    sentences = []  # dimension: num_sentence*len_sentence*2
    sentence = []
    num = 0
...
def tag_mapping(sentences):
    ...
    dico = create_dico(tags)        # 以字典的形式统计每个标签出现的次数
...
def augment_with_pretrained(dictionary, ext_emb_path, chars):
    ...
    # We either add every word in the pretrained file, or only words given in the `words` list to which
    # we can assign a pretrained embedding
    if chars is None:          # 若chars为空，将从embedding文件中抽取的pretended中出现的词而dictionary中没有的词加入dictionary
