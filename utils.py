...
def test_ner(results, path):
    ...
    with open(output_file, "w") as f:
        ...
        f.writelines(to_write)      # to_write dimension: 样本中的总字数*1  每个节点是str：字符 正确标签 预测标签
...
def create_model(session, Model_class, path, load_vec, config, id_to_char, logger):
    ...
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):                 # 若已有保存的模型文件则加载先前的模型
        logger.info("Reading model parameters from %s" % ckpt.model_checkpoint_path)
        model.saver.restore(session, ckpt.model_checkpoint_path)
    else:                                                                               # 若没有保存的模型文件则建立模型
        logger.info("Created model with fresh parameters.")
        session.run(tf.global_variables_initializer())
        if config["pre_emb"]:                                       # 如果利用预训练好的embedding
            ...
            emb_weights = load_vec(config["emb_file"],id_to_char, config["char_dim"], emb_weights)  
                                    # emb_file: Path for pre_trained embedding 
