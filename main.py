...
flags.DEFINE_integer("max_epoch",   300,        "maximum training epochs")
...
def evaluate(sess, model, name, data, id_to_tag, logger):
    ...
    ner_results = model.evaluate(sess, data, id_to_tag)  # ner_results dimension: eval/test样本数量*每句实际长度
...
def train():
    # load data sets
    train_sentences = load_sentences(FLAGS.train_file, FLAGS.lower, FLAGS.zeros)  # dimension:num_sentence*len_sentence*2
    dev_sentences = load_sentences(FLAGS.dev_file, FLAGS.lower, FLAGS.zeros)
    test_sentences = load_sentences(FLAGS.test_file, FLAGS.lower, FLAGS.zeros)

    # Use selected tagging scheme (IOB / IOBES)
    update_tag_scheme(train_sentences, FLAGS.tag_schema)    # dimension:num_sentence*len_sentence*2
    ...
    if not os.path.isfile(FLAGS.map_file):
        ...
        if FLAGS.pre_emb:                                   # 如果使用预训练的词嵌入
            dico_chars_train = char_mapping(train_sentences, FLAGS.lower)[0] # dico_chars_train dimension: 训练数据集中出现的字符类别数*2， 
    ...
    train_data = prepare_dataset(train_sentences, char_to_id, tag_to_id, FLAGS.lower) # dimension: NumSentence*4*LenSentence
    ...
    train_manager = BatchManager(train_data, FLAGS.batch_size)  # batch_data dimension: BatchNum*4*BatchSize*MaxLenSentence
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for i in range(FLAGS.max_epoch):                            # 括号中数字是epoach数量
            for batch in train_manager.iter_batch(shuffle=True):    # 一次从batch_data中取出一个batch,Shuffle为True表示打乱batch_data的顺序
...
def evaluate_line():
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        while True:
            ...
            tested_sentence = result["string"]
            entities_total = result["entities"]
            entity_PER = []
            entity_ORG = []
            entity_LOC = []
            for entity in entities_total:
                kind = entity["type"]
                if kind == 'PER':
                    entity_PER.append([entity["word"], [entity["start"], entity["end"]]])
                elif kind == 'ORG':
                    entity_ORG.append([entity["word"], [entity["start"], entity["end"]]])
                elif kind == 'LOC':
                    entity_LOC.append([entity["word"], [entity["start"], entity["end"]]])
            print(tested_sentence)
            print('Person: ', entity_PER)
            print('Orgalization: ', entity_ORG)
            print('Location: ', entity_LOC)
