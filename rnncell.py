...
class CoupledInputForgetGateLSTMCell(rnn_cell_impl.RNNCell):        # 继承了tf.nn.rnn_cell.RNNCell(为何不是LSTMCell?)
  ...
  def call(self, inputs, state):
    ...
    Args:
      ...
      state: if `state_is_tuple` is False, this must be a state Tensor,
        ...
        tuple of state Tensors, both `2-D`, with column sizes `c_state` and `m_state`.
